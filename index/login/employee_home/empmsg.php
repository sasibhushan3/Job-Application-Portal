...
*/
	...
	$query = "SELECT * FROM message where emp='$username'";
	$result = mysqli_query($con, $query);
	$numResults = mysqli_num_rows($result);

	if($numResults > 0)
	{
		echo "<center><h1>Job Applications</h1></center>";
			echo "<center><table border = '1'></center>";
			echo "<center><tr><td><b>S No.</b></td><td><b>Candidate username</b></td><td><b>Job applied</b></td><td><b>Contact Number</b></td><td><b>Skills</b></td><tr></center>";
		...
		while($row=mysqli_fetch_array($result))
		{
			...
			echo "<center><tr><td>{$i}</td><td>{$row['cand']}</td><td>{$row['job']}</td><td>{$row3['phoneno']}</td><td>{$var}</td><tr></center>";
			$i = $i+1;
		}

		echo "<center><form action = 'reply.php' method = 'POST'><input name = 'cand' placeholder = 'Candidate Name'><br><br><input name = 'job' placeholder = 'Job Name'><br><br><select name='reply'>
			  <option value='reject'>Reject job</option>
