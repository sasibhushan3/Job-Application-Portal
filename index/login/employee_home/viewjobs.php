...
*/
	...
	if($numResults > 0)
	{
		...
		while($row=mysqli_fetch_array($result))
		{
			...
			echo "<center><tr><td>{$i}</td><td>{$row['name']}</td><td>{$row['salary']}</td><td>{$row['vacancies']}</td><td>{$row['place']}</td><td>{$var}</td><tr></center>";
			$i = $i+1;
		}

		echo "<center><form action = 'delsuc.php' method = 'POST'><input name = 'job' placeholder = 'Job Name'><br><br><button type = 'submit'>Delete</button></form></center>";
